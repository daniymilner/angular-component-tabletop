...
function getEwizardVersions(callback){
...
getEwizardVersions(function(err){
	if(!err){
		getCompanies(function(err){
			if(!err){
				getComponents(function(err){
					if(!err){
						utils.writeJSONSync(path.join(__dirname, '/database.json'), sortData(), true);
						console.log('---------------');
